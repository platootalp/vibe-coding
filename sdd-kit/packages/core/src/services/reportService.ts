...
export async function composeImplementationReport(
  params: ReportParams
): Promise<{ report: SddImplementationReport; filePath: string }> {
  const report: SddImplementationReport = {
  ...
  const content = renderTemplate(params.templates.report, {
    ...
    completedPercent: Math.round(
      (params.progress.completed / Math.max(1, params.progress.completed + params.progress.inProgress)) *
        100
    ),
  ...
  const filePath = path.join(
    reportsDir,
    `sdd-report-${new Date(params.progress.timestamp).toISOString().replace(/[:.]/g, "-")}.md`
  );
