...
export function deriveTaskPlan(specification, plan) {
    const tasks = [];
...
export function applyTaskUpdates(taskPlan, updates) {
...
export function summarizeProgress(taskPlan) {
    const counts = taskPlan.tasks.reduce((acc, task) => {
        acc[task.status] += 1;
        if (task.status !== "done") {
