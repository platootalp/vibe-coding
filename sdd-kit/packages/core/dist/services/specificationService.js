...
export function generateSpecification(input) {
    ...
    const functionalThemes = buildFunctionalThemes(input.primaryModules, input.requirements);
    const nonFunctionalMatrix = buildNonFunctionalMatrix(input.nonFunctionalRequirements);
...
function buildFunctionalThemes(modules, requirements) {
    if (!modules.length) {
        return [
            {
                name: "核心体验",
    ...
    return modules.map((moduleName) => {
        const relatedReqs = requirements.filter((req) => req.title.toLowerCase().includes(moduleName.toLowerCase()));
        return {
            name: moduleName,
...
function buildNonFunctionalMatrix(requirements) {
    return requirements.map((item) => {
        const riskLevel = /99|99\.9|严格|严格|安全/i.test(`${item.attribute} ${item.metric} ${item.target}`)
            ? "high"
...
function deriveRisks(constraints, nonFunctionalMatrix) {
    const risks = constraints.map((constraint) => ({
        id: randomUUID(),
        title: constraint,
