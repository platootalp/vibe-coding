...
async function loadJson<T>(filePath: string, hint: string): Promise<T> {
  ...
  try {
    ...
    throw new Error(`${hint} JSON 解析失败: ${(error as Error).message}`);
  }
}
...
program
  ...
  .action(async (options: { name: string; domain: string; summary: string; output: string }) => {
    await runAction(async () => {
      const initOptions: SddInitOptions = {
...
program
  ...
  .option("-p, --principle <value>", "指导原则，可重复传入", collectValues, [] as string[])
  .requiredOption("-g, --governance <string>", "治理模型描述")
  .requiredOption("-c, --cadence <string>", "交付节奏")
  .action(async (options: { principle: string[]; governance: string; cadence: string }) => {
    await runAction(async () => {
      const filePath = await runWithSpinner("更新宪章", () =>
        engine.updateConstitution({
...
program
  ...
  .action(async (options: { file: string }) => {
    await runAction(async () => {
      const overrides = await loadJson<Partial<SddTemplateRegistry>>(options.file, "模板配置");
...
program
  ...
  .action(async (options: { input: string }) => {
    await runAction(async () => {
      const payload = await loadJson<SddSpecificationInput>(options.input, "规格输入");
  ...
  .action(async (options: { spec?: string }) => {
    await runAction(async () => {
      const specPayload = options.spec
        ? await loadJson<SddSpecificationInput>(options.spec, "规范输入")
        : undefined;
      const planOptions: SddPlanOptions = {};
      if (specPayload) {
...
program
  ...
  .action(async (options: { plan?: string }) => {
    await runAction(async () => {
      const planOptions: SddTasksOptions = {};
      if (options.plan) {
        planOptions.plan = await loadJson<SddTechnicalPlan>(options.plan, "技术方案");
      }
...
program
  ...
  .action(
    async (options: { updates?: string; highlights?: string; blockers?: string }) => {
      await runAction(async () => {
        const payload: SddImplementationInput = {};
        if (options.updates) {
          payload.updates = await loadJson<SddImplementationInput["updates"]>(
            options.updates,
            "任务更新"
          );
        }
